...
def apply_preset(cfg: FullModelConfig, name: str):
    ...
    # Save current mode values before reset
    saved_modes = {
        "l5_mode": cfg.preset_modes.l5_mode,
        "purkinje_mode": cfg.preset_modes.purkinje_mode,
        "delay_target": cfg.preset_modes.delay_target,
        "anesthesia_mode": cfg.preset_modes.anesthesia_mode,
        "k_mode": cfg.preset_modes.k_mode,
        "alzheimer_mode": cfg.preset_modes.alzheimer_mode,
        "hypoxia_mode": cfg.preset_modes.hypoxia_mode,
        "ach_mode": cfg.preset_modes.ach_mode,
        "dravet_mode": cfg.preset_modes.dravet_mode,
    }
    ...
    # --- 7. ПАТОЛОГИЯ: РАССЕЯННЫЙ СКЛЕРОЗ (Демиелинизация) ---
        ...
        cfg.morphology.gNa_trunk_mult = 0.2  # Reduced sodium in internodes (nav depletion)

        cfg.morphology.Ra = 600.0  # Elevated axial resistance
    ...
    # Extract preset code for mode application
    code = _preset_code(name)
